...
const generarNuevoTexto = () => {
  ...
  charStatuses.value = Array.from({ length: textToType.value.length }, () => 'pending');
  everIncorrect.value = Array.from({ length: textToType.value.length }, () => false);
};
...
const startCountdown = () => {
  ...
  countdownTimer = setInterval(async () => {
    if (countdownNumber.value > 1) {
      ...
      try { typingInputRef.value?.focus(); } catch {}
    }
  }, 1000);
};
...
const updateStatuses = (typed) => {
  ...
  progress.value = Math.floor((Math.min(typed.length, expected.length) / expected.length) * 100);
};
...
const updatePlayerProgress = (data) => {
  const { playerId, progress: playerProgress, wpm, errors: playerErrors } = data;
          ...
          <div class="text-display">
            <p class="text-to-type">
              <span 
                ...
                :key="idx" 
                :class="[statusClass(charStatuses[idx]), { 'is-space': ch === ' ' }]"
              >
                <!-- Indicadores de posición de otros jugadores -->
                <span 
                  v-for="(player, pIndex) in playersState.filter(p => getPlayerIndicator(p.progress) === idx)" 
